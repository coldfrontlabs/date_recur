 ...
 *
 * @FieldWidget(
 *   id = "date_recur_advanced_widget",
 *   label = @Translation("Date recur advanced widget"),
 *   field_types = {
 *     "date_recur"
 *   }
 * )
 */
class DateRecurAdvancedWidget extends DateRecurDefaultWidget {
  /**
   * {@inheritdoc}
   */
  ...
  public static function defaultSettings() {
    return [
        'allowed_repeat_types' => array_keys(self::getRepeatOptions()),
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    // Restrict repeat options to formatter options.  Allow existing options outside of formatter.
    $repeat_options = self::getRepeatOptions();
    ...
    if (!$values['repeat']) {
      ...
      if($date_value instanceof DrupalDateTime) {
        $date_value =$date_value->format(DATETIME_DATE_STORAGE_FORMAT);
      }
...
//        '#date_date_format' => DATETIME_DATE_STORAGE_FORMAT,
//        '#wrapper_attributes' => [
//          'class' => [
//            'container-inline',
//            'form-inline',
...
//        ],
//        '#attributes' => ['class' => ['container-inline', 'form-inline']],
////        '#default_value' => $values['repeat_settings']['until']['date']['until_date'],
...
//    }
  ...
  public static function getValueFromRRule($rrule) {
    $test = new DateRecurRRule($rrule, new DrupalDateTime());
    $parts = $test->getParts();
    $values =  [
      'repeat' => $parts['FREQ'],
      'repeat_settings' => [
  ...
  public static function getRRuleFromValue($value) {
    ...
    switch($value['repeat_settings']['until_op']) {
      case 'count':
        ...
        break;
      case 'date':
        $date = DrupalDateTime::createFromFormat(DATETIME_DATE_STORAGE_FORMAT, $value['repeat_settings']['until']['date']['until_date']);
        $parts['UNTIL'] = $date;
        break;
    }
